<template>
  <table class="w-full mx-auto text-sm mt-5">
    ...
    <tbody>
      <tr v-for="(sale, index) in shiftSales" :key="sale.id" class="mb-4" :class="{ 'bg-[#EDEDED]': index % 2 == 0 }">
  ...
  <DialogModal :show="show_edit_sale_modal" @close="show_edit_sale_modal = false">
    <template #title>
      <p class="text-sm font-bold">Editar venta <span class="text-primary font-bold text-sm ml-3">{{
        edit_sale.product.name }}</span></p>
    </template>
    <template #content>
      <div class="mb-3 w-full">
        <InputLabel value="Cantidad *" class="ml-3 mb-1 text-sm" />
        <input v-model="form.quantity" type="number" autocomplete="off" required class="input" placeholder="00" />
      </div>
