<template>
  <AppLayout title="Historial de Solicitudes de consumibles">
    <template #header>
      <div class="flex items-center mt-2">
        <Back />
        <h2 class="font-semibold text-xl text-gray-800 text-center ml-5 lg:ml-28">
          Historial de Solicitudes de consumibles
        </h2>
      </div>
    </template>

    <div class="globe-container flex-col">
        <Link :href="route('consumable-request.show', request)" v-for="request in requests.data" :key="request.id"
